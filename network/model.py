...
class ContrastDrivenFeatureAlignment(nn.Module):
    def __init__(self, in_channel, out_channel, num_heads, window_size=3, window_padding=1, window_stride=1, attn_drop=0., proj_drop=0.):
        super().__init__()
        '''
        ...
        self.v = nn.Linear(out_channel, out_channel)
        self.attn_fg = nn.Linear(out_channel, window_size ** 4 * num_heads)
        self.attn_bg = nn.Linear(out_channel, window_size ** 4 * num_heads)
        self.input_ori = nn.Linear(in_channel, out_channel)
    ...
    def forward(self, x, fg, bg):
        ...
        input shape: (B,C,H,W)
        '''
        #adjust the shape of feature maps
        x = x.permute(0, 2, 3, 1)
        ...
        #B,C,H,W -> B,C*K*K,H*W  -> B,nh,h,K*K,H*W -> B,nh,,H*W,K*K,h
        v_unfolded = self.unfold(v).reshape(B, self.num_heads, self.head_dim,
                                            self.window_size * self.window_size,
                                            -1).permute(0, 1, 4, 3, 2)
        #B,nh,H*W,K*K,K*K
        attn_fg = self.compute_attention(fg, B, H, W, C, 'fg') 
        #utilize the forground attention weights to guide model
        #B,H,W,C
        x_weighted_fg = self.apply_attention(attn_fg, v_unfolded, B, H, W, C)
        
        #B,nh,H*W,K*K,K*K
        attn_bg = self.compute_attention(bg, B, H, W, C, 'bg')
        #utilize the background attention weights to guide model
        #B,H,W,C
        x_weighted_bg = self.apply_attention(attn_bg, v_unfolded, B, H, W, C) 
        
        x_weighted = x_weighted_fg + x_weighted_bg
    ...
    def compute_attention(self, feature_map, B, H, W, C, feature_type):
        ...
        h, w = math.ceil(H / self.window_stride), math.ceil(W / self.window_stride)
        feature_map_pooled = self.pool(feature_map.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        #B,H,W,K*K*K*K*nh -> B,H*W,nh,K*K,K*K -> B,nh,H*W,K*K,K*K
        attn = attn_layer(feature_map_pooled).reshape(B, h * w, self.num_heads,
                                                      self.window_size * self.window_size,
                                                      self.window_size * self.window_size).permute(0, 2, 1, 3, 4)#B,H*W,nh,K*K,K*K
    ...
    def apply_attention(self, attn, v, B, H, W, C):
        #B,nh,H*W,K*K,h -> B,bh,h,K*K,H*W -> B,C*K*K,H*W
        #calculate the attention
        x_weighted = (attn @ v).permute(0, 1, 4, 3, 2).reshape(
            B, self.dim * self.window_size * self.window_size, -1)
        #Fold feature maps to get neiborhood attention values
        # B,C,H,W
        x_weighted = F.fold(x_weighted, output_size=(H, W), kernel_size=self.window_size,
